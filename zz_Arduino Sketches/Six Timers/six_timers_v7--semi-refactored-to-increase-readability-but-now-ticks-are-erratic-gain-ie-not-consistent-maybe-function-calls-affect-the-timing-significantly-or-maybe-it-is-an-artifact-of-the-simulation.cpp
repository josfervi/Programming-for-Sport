...
#define MODE_SWITCH_BUTTON_PIN 9 // starts/pauses the first non-zero duration task timer
                                 // impl detal: treated as task0 button pin
...
const int BUTTON_PIN_OF_TASK[NUMBER_OF_TASKS+1] = {MODE_SWITCH_BUTTON_PIN, TASK1_BUTTON_PIN, TASK2_BUTTON_PIN, TASK3_BUTTON_PIN, TASK4_BUTTON_PIN};
...
unsigned long debouncingTimer_elapsedTime [NUMBER_OF_TASKS+1] = {0UL, 0UL, 0UL, 0UL, 0UL}; // TODO: is the default array value 0?
unsigned long debouncingTimer_startTime   [NUMBER_OF_TASKS+1]=  {0UL, 0UL, 0UL, 0UL, 0UL}; // TODO: is the default array value 0?

unsigned int taskDuration [NUMBER_OF_TASKS+1] = {NULL, 0, 0, 0, 0}; // task 1 duration increments by INCREMENT_TASK_DURATION_BY_THIS_AMOUNT each time the task 1 button is pressed, same is true for each button
...
void loop() {
  ...
  for (int task_number= 0; task_number <= NUMBER_OF_TASKS; task_number++) {
    
    const BYTE BITN= 1 << task_number;
    
    BYTE current_reading= digitalRead(BUTTON_PIN_OF_TASK[ task_number]);
    ...
    if (reading_changed) {
      ...
      debouncingTimer_elapsedTime[task_number]= 0;
      debouncingTimer_startTime[task_number]=   millis();
    }
    else {
      debouncingTimer_elapsedTime[task_number]= millis() - debouncingTimer_startTime[task_number];
      
      if (debouncingTimer_elapsedTime[task_number] > DEBOUNCE_TIME) {
        ...
        if ( (lastButtonState == NOT_PRESSED) && (buttonState == PRESSED) ) {
          ...
          if ( task_number == 0 ) {
            ...
            taskDuration[task_number]+= INCREMENT_TASK_DURATION_BY_THIS_AMOUNT;
            int col_num= (task_number - 1) * 3; 
            lcd.setCursor(col_num, LCD_SECOND_LINE);
            lcd.print(taskDuration[task_number]);
          }
        }
      }
    }
  ...
  // Control LEDs through the Shift Register
  
  // take the latch pin low so 
  // the LEDs don't change while you're sending in bits:
  digitalWrite(SR_LATCH_PIN, HOLD);
  // shift out the bits:
  shiftOut(SR_DATA_PIN, SR_CLOCK_PIN, MSBFIRST, ledStates);
  
  //take the latch pin high so the LEDs will light up:
  digitalWrite(SR_LATCH_PIN, GO);
...
void countdown() {
  ...
  int idx= getIdxOfFirstTaskWithNonzeroDuration();
  
  if ( idx == -1) {
    return;
  }
    
  int validTask_number= idx;
  
  // start counting down
  
  timeDelta= millis() - prevTime;
  
  if (timeDelta > 30UL) {
    prevTime= millis();
  	taskDuration[validTask_number]--;
  	
  	printUpdatedTaskDurationToLCD(validTask_number);
  }
}

int getIdxOfFirstTaskWithNonzeroDuration() {
  int idx= 1;
  while (idx <= NUMBER_OF_TASKS && taskDuration[idx] == 0) {
    idx++;
  }
...
void printUpdatedTaskDurationToLCD(int task_number) {
  int lcd_col_num= (task_number - 1) * 3;
	if ( taskDuration[task_number] < 10 ) {
    lcd.setCursor(lcd_col_num, LCD_SECOND_LINE);
    lcd.print(" ");
	  lcd_col_num++;
  }
	lcd.setCursor(lcd_col_num, LCD_SECOND_LINE);
