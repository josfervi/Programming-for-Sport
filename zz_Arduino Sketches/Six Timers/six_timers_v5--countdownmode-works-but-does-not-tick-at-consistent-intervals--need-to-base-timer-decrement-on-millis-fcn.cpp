...
void loop() {
  ...
  if (countdown_mode) {
    ...
    if (idx <= NUMBER_OF_TASKS) {
      ...
      taskDuration[idx]--;
      int col_num= (idx - 1) * 3;
      if ( taskDuration[idx] < 10 ) {
        lcd.setCursor(col_num, LCD_SECOND_LINE);
        lcd.print(" ");
      	col_num++;
      }
      lcd.setCursor(col_num, LCD_SECOND_LINE);
      lcd.print(taskDuration[idx]);
      delay(100); // TODO: instead of delaying, decrement taskDuration by a decimal amount, then display int( taskDuration )
    }
  }
